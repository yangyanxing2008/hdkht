...
function updateshow(id) {
    for(var i=0;i<positionals.length;i++){
        if(id==positionals[i].id){
            $("#positionalName").val(positionals[i].positionalName);
            break;
        }
    }
    $(".updateE").attr("did",id);
...
function updateDepart() {
    var d = {};
    d.positionalName = $("#positionalName").val();
    d.state = 0;
    d.id = $(".updateE").attr("did");
    $.ajax({
        type: "post",
        url: "/cyyht/positional/update.action",
...
function  addDepart() {
    ...
    department.positionalName= $("#positionalContent").val();
    $.ajax({
        type: "post",
        url: "/cyyht/positional/save.action",
        contentType: 'application/json',
...
function jyPositional(id) {
    if(window.confirm('你确定要禁用该职称吗？')){
        var d = {};
        d.id = id;
        for(var i=0;i<positionals.length;i++){
            if(id==positionals[i].id){
                d.positionalName =positionals[i].positionalName ;
                d.state = 1;
                break;
            }
        }
...
function deletep(id){
    if(window.confirm('你确定要删除吗？')){
        ...
        $.ajax({
            type: "post",
            url: "/cyyht/positional/delete.action",
            contentType: 'application/json',
            data: JSON.stringify(d),
            dataType: "json",
            success: function (data) {
...
function getData(){
    ...
    $.ajax({
        type:"post",
        url:"/cyyht/positional/getPageList.action",
        contentType: 'application/json',
            ...
            if(data.code ==1){
                ...
                if(data.data.length >0){
                    ...
                    for ( var i = 0; i < lists.length; i++) {
                        ...
                        td3.append(lists[i].positionalName);
